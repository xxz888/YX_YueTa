...
@property (nonatomic,strong) UIView *maskView;           //黑色背景视图
...
+ (void)showPickerWithTitle:(NSString *)title dataSource:(NSArray<NSString *> *)dataSource defaultStr:(NSString *)defaultStr valueDidSelectBlock:(void (^)(NSString *, NSInteger))block {
    ValuePickerView *picker = [[ValuePickerView alloc] init];
    picker.pickerTitle = title;
    picker.dataSource = dataSource;
    if (defaultStr.length) {
        picker.defaultStr = defaultStr;
    }
    picker.valueDidSelect = ^(NSString *value) {
        NSArray * stateArr = [value componentsSeparatedByString:@"/"];
        NSString *state = stateArr[0];
...
- (void)setDefaultStr:(NSString *)defaultStr
{
    ...
    NSInteger index = [self.dataSource indexOfObject:defaultStr];
    
    _stateStr = [NSString stringWithFormat:@"%@/%@",defaultStr,@(index)];
