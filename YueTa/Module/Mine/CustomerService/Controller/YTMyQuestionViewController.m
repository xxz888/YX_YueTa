...
@interface YTMyQuestionViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation YTMyQuestionViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setNavigationBarTitle:@"我的提问"];
    ...
    [MineInterface getMyProblemListAndBlock:^(ResponseMessage * _Nonnull rspStatusAndMessage, NSArray<YTMyProblemModel *> * _Nonnull problemList) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *status = model.status == 0 ? @"未解答" : @"已解答";
    cell.textLabel.text = [NSString stringWithFormat:@"(%@)%@",status,model.problem];
