...
@interface YTFiltrateSquareController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic, strong) UIButton *curGenderBtn;
...
@property (nonatomic, assign) NSInteger gender;//男的是0，女的是1，不限是2
@property (nonatomic, assign) NSInteger max_reward;
@property (nonatomic, assign) NSInteger min_reward;

@end

@implementation YTFiltrateSquareController
...
- (void)genderButtonClick:(UIButton *)button {
    button.selected = YES;
    self.curGenderBtn.selected = NO;
    self.curGenderBtn = button;
    self.gender = button.tag;
}

- (void)continueButtonClick {
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(YTFiltrateControllerCompleteWithMax_weight:min_weight:max_age:min_age:max_height:min_height:isVip:auth_status:sort:work:education:show:gender:max_reward:min_reward:)]) {
        
        [self.delegate YTFiltrateControllerCompleteWithMax_weight:self.max_weight min_weight:self.min_weight max_age:self.max_age min_age:self.min_age max_height:self.max_height min_height:self.min_height isVip:self.isVip auth_status:self.auth_status sort:self.sort work:self.work education:self.education show:self.show gender:self.gender max_reward:self.max_reward min_reward:self.min_reward];
    }
...
- (void)createUI {
    ...
    // 打赏
    UILabel *moneyLab = [UILabel labelWithName:@"打赏" Font:kSystemFont15 textColor:kBlackTextColor textAlignment:NSTextAlignmentLeft];
    moneyLab.frame = CGRectMake(kRealValue(10), sliderW.bottom+kRealValue(10), kRealValue(100), kRealValue(20));
    [self.headerView addSubview:moneyLab];
    
    UILabel *moneyLeft = [UILabel labelWithName:@"10元" Font:kSystemFont12 textColor:kBlackTextColor textAlignment:NSTextAlignmentLeft];
    moneyLeft.frame = CGRectMake(kRealValue(20), moneyLab.bottom, kRealValue(100), kRealValue(20));
    [self.headerView addSubview:moneyLeft];
    
    UILabel *moneyRight = [UILabel labelWithName:@"400元" Font:kSystemFont12 textColor:kBlackTextColor textAlignment:NSTextAlignmentRight];
    moneyRight.frame = CGRectMake(kScreenWidth-kRealValue(110), moneyLab.bottom, kRealValue(100), kRealValue(20));
    [self.headerView addSubview:moneyRight];
    
    SDRangeSliderView *sliderMoney = [[SDRangeSliderView alloc] initWithFrame:CGRectMake(kRealValue(10), moneyRight.bottom+kRealValue(10), kScreenWidth-kRealValue(20), 50)];
    sliderMoney.lineColor = kGrayBorderColor;
    sliderMoney.highlightLineColor = RGB(192, 164, 234);
    [self.headerView addSubview:sliderMoney];
    sliderMoney.minValue = 30;
    sliderMoney.maxValue = 120;
    sliderMoney.leftValue = 30;
    sliderMoney.rightValue = 120;
    [sliderMoney eventValueDidChanged:^(double left, double right) {
        moneyLeft.text = [NSString stringWithFormat:@"%@元",@(left)];
        moneyRight.text = [NSString stringWithFormat:@"%@元",@(right)];
        self.max_reward = (NSInteger)right;
        self.min_reward = (NSInteger)left;
    }];
    
    // VIP
    UILabel *VIPLab = [UILabel labelWithName:@"VIP" Font:kSystemFont15 textColor:kBlackTextColor textAlignment:NSTextAlignmentLeft];
    VIPLab.frame = CGRectMake(kRealValue(10), sliderMoney.bottom+kRealValue(10), kRealValue(100), kRealValue(20));
    ...
    // 性别
    UILabel *genderLab = [UILabel labelWithName:@"性别" Font:kSystemFont15 textColor:kBlackTextColor textAlignment:NSTextAlignmentLeft];
    genderLab.frame = CGRectMake(kRealValue(10), sortRight.bottom+kRealValue(10), kRealValue(100), kRealValue(20));
    [self.headerView addSubview:genderLab];
    
    UIButton *allGender = [UIButton buttonWithTitle:@"全部" taget:self action:@selector(genderButtonClick:) font:kSystemFont14 titleColor:kBlackTextColor];
    allGender.frame = CGRectMake(kRealValue(20), genderLab.bottom+kRealValue(10), kRealValue(60), kRealValue(30));
    [allGender setTitleColor:kWhiteTextColor forState:UIControlStateSelected];
    allGender.layer.cornerRadius = sortLeft.height/2;
    allGender.layer.masksToBounds = YES;
    allGender.layer.borderColor = kGrayBorderColor.CGColor;
    allGender.layer.borderWidth = 1.f;
    allGender.tag = 2;
    [allGender setBackgroundImage:[self createImageWithColor:RGB(255, 255, 255)] forState:UIControlStateNormal];
    [allGender setBackgroundImage:[self createImageWithColor:RGB(192, 164, 234)] forState:UIControlStateSelected];
    allGender.selected = YES;
    [self.headerView addSubview:allGender];
    self.curGenderBtn = allGender;
    
    UIButton *genderMale = [UIButton buttonWithTitle:@"男" taget:self action:@selector(genderButtonClick:) font:kSystemFont14 titleColor:kBlackTextColor];
    genderMale.frame = CGRectMake(allGender.right + kRealValue(50), genderLab.bottom+kRealValue(10), kRealValue(60), kRealValue(30));
    [genderMale setTitleColor:kWhiteTextColor forState:UIControlStateSelected];
    genderMale.layer.cornerRadius = genderMale.height/2;
    genderMale.layer.masksToBounds = YES;
    genderMale.layer.borderColor = kGrayBorderColor.CGColor;
    genderMale.layer.borderWidth = 1.f;
    genderMale.tag = 0;
    [genderMale setBackgroundImage:[self createImageWithColor:RGB(255, 255, 255)] forState:UIControlStateNormal];
    [genderMale setBackgroundImage:[self createImageWithColor:RGB(192, 164, 234)] forState:UIControlStateSelected];
    [self.headerView addSubview:genderMale];
    
    
    UIButton *genderfemale = [UIButton buttonWithTitle:@"女" taget:self action:@selector(genderButtonClick:) font:kSystemFont14 titleColor:kBlackTextColor];
    genderfemale.frame = CGRectMake(genderMale.right + kRealValue(50), genderLab.bottom+kRealValue(10), kRealValue(60), kRealValue(30));
    [genderfemale setTitleColor:kWhiteTextColor forState:UIControlStateSelected];
    genderfemale.layer.cornerRadius = genderfemale.height/2;
    genderfemale.layer.masksToBounds = YES;
    genderfemale.layer.borderColor = kGrayBorderColor.CGColor;
    genderfemale.layer.borderWidth = 1.f;
    genderfemale.tag = 1;
    [genderfemale setBackgroundImage:[self createImageWithColor:RGB(255, 255, 255)] forState:UIControlStateNormal];
    [genderfemale setBackgroundImage:[self createImageWithColor:RGB(192, 164, 234)] forState:UIControlStateSelected];
    [self.headerView addSubview:genderfemale];
    
    self.headerView.height = genderfemale.bottom+kRealValue(10);
